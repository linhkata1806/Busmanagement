...
public class CreateNotificationServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        request.getRequestDispatcher("/view/staff/notification/create-notification.jsp").forward(request, response);
    }
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        try {
            ...
            if (notificationService.createNotification(accountId, type, title, content)) {
                ...
                request.setAttribute("msgError", "Dữ liệu không hợp lệ. Vui lòng kiểm tra lại!");
                request.getRequestDispatcher("/view/staff/notification/create-notification.jsp").forward(request, response);
            }
        } catch (NumberFormatException e) {
            request.setAttribute("msgError", "Mã khách hàng phải là một số hợp lệ!");
            request.getRequestDispatcher("/view/staff/notification/create-notification.jsp").forward(request, response);
        }
    }
